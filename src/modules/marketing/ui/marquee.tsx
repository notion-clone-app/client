...
export const Marquee: React.FC<SingleMarqueeProps> = ({
  ...
  const baseExtendedRow = Array.from({ length: multiplier }, () => items).flat();
  const finalInfiniteRow = [...baseExtendedRow, ...baseExtendedRow];

  return (
    <div className={`relative w-full overflow-hidden py-2 ${pauseOnHover ? "marquee-parent" : ""}`}>
      ...
      <div className="flex w-max items-center">
        ...
        >
          {finalInfiniteRow.map((item, index) => (
            <div key={`marquee-item-${index}`} className="flex items-center justify-center">
              {item}
            </div>
          ))}
        </div>
