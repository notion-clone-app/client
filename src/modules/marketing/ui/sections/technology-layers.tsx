...
const TechCard: FC<{
  ...
  return (
    ...
    >
      <div
        className={`relative flex flex-col gap-3 rounded-xl border border-border/40 bg-card/50 p-4 transition-all duration-300 hover:-translate-y-1 hover:border-border hover:bg-secondary/30 hover:shadow-lg dark:bg-white/[0.02] dark:hover:bg-white/[0.05]`}
      >
        {/* Icon */}
        <div
          className={`flex h-10 w-10 items-center justify-center rounded-lg bg-secondary/80 dark:bg-white/5 group-hover:${accentColor} transition-colors duration-300`}
        >
          <Icon
            className={`h-5 w-5 text-muted-foreground group-hover:${accentColor} transition-colors duration-300`}
          />
        </div>
...
const LayerRow: FC<{
  ...
  return (
    <motion.div ref={ref} className="relative" style={{ y: yOffset }}>
      {/* Layer header */}
      <motion.div
      ...
      >
        <div className={`h-px max-w-12 flex-1 bg-gradient-to-r ${layer.bgColor}`} />
...
export const TechnologyLayers: FC = () => {
  ...
  return (
    <section className="overflow-hidden py-24 md:py-32" ref={sectionRef}>
      <LandingContainer>
        <div className="flex flex-col gap-16">
          ...
          <div className="flex flex-col gap-12 md:gap-16">
            {LAYERS.map((layer, i) => (
              <LayerRow key={layer.id} layer={layer} index={i} yOffset={yOffsets[i] ?? y2} />
            ))}
          </div>
        </div>
      </LandingContainer>
