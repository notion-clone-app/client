...
export const Input: FC<Props> = ({ className, type, hasErrors, messages, labelContent: labelNode, id: providedId, "aria-describedby": describedBy, ...props }) => {
    const generatedId = useId()
    const id = providedId ?? generatedId
    const messageId = `${id}-message`
    return (
        <>
            {labelNode && <label className={cn({ 'text-destructive': hasErrors })} htmlFor={id}>{labelNode}</label>}
            <input
                id={id}
                type={type}
                data-slot="input"
            ...
            />
            {messages && (
                <div id={messageId} className="mt-1 text-xs text-destructive">{messages}</div>
            )}
        </>
    )
}
